...
class CancelledJobOrdersActivity : BaseActivity() {
    ...
    private val TAG = "CancelledJobOrdersActivity"

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_cancelled_job_orders)
        
        // Initialize views
        initViews()
        
        // Load all cancelled payment orders
        loadCancelledPaymentOrders()
    }
    
    private fun initViews() {
        recyclerView = findViewById(R.id.recyclerViewCancelledJobOrders)
        ...
        supportActionBar?.title = "Cancelled Job Orders"
    ...
    private fun loadCancelledPaymentOrders() {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (payments != null && payments.isNotEmpty()) {
                        // Filter only cancelled payments
                        val cancelledPayments = payments.filter { payment ->
                            ...
                            // Only include orders that are truly cancelled
                            effectiveStatus.equals("Cancelled", ignoreCase = true)
                        }
                        
                        if (cancelledPayments.isNotEmpty()) {
                            setupCancelledPaymentsAdapter(cancelledPayments)
                    ...
                    Toast.makeText(this@CancelledJobOrdersActivity, errorMessage, Toast.LENGTH_LONG).show()
                    tvNoOrders.visibility = View.VISIBLE
                }
            } catch (e: IOException) {
                // Handle network error
                Log.e(TAG, "Network error: ${e.message}")
                Toast.makeText(this@CancelledJobOrdersActivity, "Network error: ${e.message}", Toast.LENGTH_LONG).show()
                tvNoOrders.visibility = View.VISIBLE
            } catch (e: HttpException) {
                // Handle HTTP exception
                Log.e(TAG, "HTTP error: ${e.message}")
                Toast.makeText(this@CancelledJobOrdersActivity, "HTTP error: ${e.message}", Toast.LENGTH_LONG).show()
                tvNoOrders.visibility = View.VISIBLE
            } catch (e: Exception) {
                // Handle other exceptions
                Log.e(TAG, "Error: ${e.message}")
                Toast.makeText(this@CancelledJobOrdersActivity, "Error: ${e.message}", Toast.LENGTH_LONG).show()
                tvNoOrders.visibility = View.VISIBLE
            }
        }
    }
